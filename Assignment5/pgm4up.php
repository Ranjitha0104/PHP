...
include 'con_db.php';
    if($_POST)
    {
        ...
        if($row==0){
            echo '<script>alert("Passport Number not found");window.location="pgm4update.php"</script>';
        }
    }
?>
...
$sql = mysqli_query($con,"UPDATE passport set `passno`='$passno',`fname`='$fname',`mname`='$mname',`lname`='$lname',`dob`='$dob',`nation`='$nation',`address`='$addr',`gender`='$gender' WHERE passno='$pno'");
if($sql)
                    {
                        echo '<script>alert("Data Updated Successfully");window.location="pgm4view.php"</script>';
                    }
                    else
                    {
