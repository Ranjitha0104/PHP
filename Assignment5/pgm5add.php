...
$type = $_POST['type'];
$bal = $_POST['balance'];
$q =mysqli_query($con, "select * from bank WHERE acno = $acno ");
        $row=mysqli_fetch_array($q);
if($row==0){
if($bal>=500)
{
$sql = mysqli_query($con,"INSERT INTO bank VALUES('$acno','$name','$type','$addr','$bal')");
if($sql)
                    {
                        echo '<script>alert("Data Added Successfully");window.location="pgm5view.php"</script>';
                    }
                    else
                    {
...
{
  echo '<script>alert("Minimum 500 Rupees should be maintained");window.location="pgm5add.php"</script>';  
}
}
else{
    echo '<script>alert("Account Number Already Present");window.location="pgm5.php"</script>';
}
}
?>
</body>
